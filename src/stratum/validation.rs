...
pub fn build_candidate_block(
    ...
    // Build the header using the shared primitive with template values
    ...
    // Update size and rebuild header with correct size
    let first_serialization = block.ser();
    let size = first_serialization.len() as u64;
    
    let final_header_bytes = build_stratum_header(
        &job.coinbase1,
        extranonce1,
        &sub.extranonce2,
        &job.coinbase2,
    ...
    .map_err(|e| anyhow::anyhow!("header build error with size: {}", e))?;
    
    let mut header_bytes_for_deser = Bytes::from_slice(&final_header_bytes);
    block.header = LotusHeader::deser(&mut header_bytes_for_deser)
        .map_err(|e| anyhow::anyhow!("header deser error: {}", e))?;
...
/// Build a candidate block and compute the block hash using the stratum header
/// with the template's height, epoch_hash, extended_metadata_hash, and size.
/// This ensures the hash matches what lotusd will compute when validating.
///
/// Returns (candidate_block_for_submission, stratum_hash_hex, stratum_merkle_root_hex).
pub fn build_candidate_block_with_stratum_hash(
    ...
    let header_bytes = build_stratum_header(
        ...
        Some(&job.epoch_hash_hex),
        Some(&job.extended_metadata_hash_hex),
        None, // size will be updated below
    )
    .map_err(|e| anyhow::anyhow!("header build error: {}", e))?;

    // Compute hash from the header - now includes correct template values
    let stratum_header_hash = LotusHeader::deser(&mut Bytes::from_slice(&header_bytes))
        .map_err(|e| anyhow::anyhow!("header deser error: {}", e))?;
    let stratum_hash = stratum_header_hash.calc_hash();
    let _stratum_hash_hex = stratum_hash.to_hex_be();
    let _stratum_merkle_root_hex = stratum_header_hash.merkle_root.to_hex_be();
    ...
    // Update the block size field to match the actual serialized size
    let first_serialization = block.ser();
    block.header.size = first_serialization.len() as u64;
    
    // Rebuild header with updated size for final hash
    let final_header_bytes = build_stratum_header(
        &job.coinbase1,
        extranonce1,
        &sub.extranonce2,
        ...
        Some(&job.epoch_hash_hex),
        Some(&job.extended_metadata_hash_hex),
        Some(block.header.size),
    )
    .map_err(|e| anyhow::anyhow!("header build error with size: {}", e))?;
    
    // Recompute hash with correct size
    let final_header = LotusHeader::deser(&mut Bytes::from_slice(&final_header_bytes))
        .map_err(|e| anyhow::anyhow!("header deser error: {}", e))?;
    let final_hash = final_header.calc_hash();
    let final_hash_hex = final_hash.to_hex_be();
    let final_merkle_root_hex = final_header.merkle_root.to_hex_be();
    
    // Update block with final header
    block.header = final_header;
    let final_serialization = block.ser();
    
    Ok((final_serialization.as_ref().to_vec(), final_hash_hex, final_merkle_root_hex))
}
