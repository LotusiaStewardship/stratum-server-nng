...
impl NetworkDifficultyTracker {
    ...
    /// Extracts target, converts to difficulty, updates network diff.
    pub fn update_from_template(&self, template: &MiningTemplate) {
        ...
        let network_diff = target_to_difficulty(&target_bytes).unwrap();
...
mod tests {
    ...
    fn test_tracker_default_config() {
        ...
        assert!((config.vardiff_target_secs - 15.0).abs() < 0.0001);
        assert!((config.vardiff_retarget_secs - 90.0).abs() < 0.0001);
    }
