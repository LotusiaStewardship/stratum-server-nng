...
pub fn release_scheduler_lease(db: &AccountingDb, instance_id: &str) -> Result<bool> {
...
mod tests {
    ...
    fn create_test_db() -> (TempDir, AccountingDb) {
        ...
        let db = AccountingDb::open(db_path.to_str().unwrap()).expect("failed to open db");
    ...
    fn test_lease_contention_multiple_instances() {
        ...
        for i in 1..=5 {
            ...
            let result =
                acquire_scheduler_lease(&db, &instance_id, DEFAULT_LEASE_DURATION_MINS).unwrap();
    ...
    fn test_acquire_renew_release_lifecycle() {
        ...
        for _ in 0..3 {
            let renewed =
                renew_scheduler_lease(&db, "instance-1", DEFAULT_LEASE_DURATION_MINS).unwrap();
            assert!(renewed);
        }
